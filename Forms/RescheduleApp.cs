...
{
    public partial class RescheduleApp : Form
    {
        ...
        private int prevScheduleId;

        public RescheduleApp(int appId)
        {
            InitializeComponent();
            AppointmentId = appId;
            con = new DbAccess.DbConnection();
            string PatientQry = "SELECT * FROM tbl_appointments WHERE appointment_id ='" + appId + "'";
            selectedPatientId = con.GetColumnValue(PatientQry, "patient_id") as int? ?? 0;
            prevScheduleId = con.GetColumnValue(PatientQry, "schedule_id") as int? ?? 0;

            LoadClinicData();
            Doctor_Load();
            Clinic_Load();
            this.FormClosing += RescheduleApp_FormClosing;
        ...
        private void LoadClinicData()
        {
            try
            {
                List<Clinic> clinics = Clinic.GetAllClinicsFromDatabase(con, "AND status!='Completed' AND schedule_id!='" + prevScheduleId + "' ");
                if (clinics == null)
                {
        ...
        private void RescheduleApp_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = true;
            this.Hide();
        }
        ...
        private void Submit_Click(object sender, EventArgs e)
        {
            ...
            app.UpdateAppointment(con);

            // Call the common method to handle printing
            HandlePrinting(null); // Pass null or any necessary data if needed for printing
            MessageBox.Show("Appointment Rescheduled Successfully... Booking Ticket has mailed...", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            this.Hide();
        }
        ...
        private void ClinicType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (clinicType > 0)
            {
                try
                {
                    string clinic_exist = "SELECT * FROM tbl_schedule WHERE clinic_id='" + clinicType + "' AND status!='removed' AND status!='Completed' AND schedule_id!='" + prevScheduleId + "' ";
        ...
        private void Doctor_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (doctorType > 0)
            {
                try
                {
                    string doctor_exist = "SELECT * FROM tbl_schedule WHERE doctor_id='" + doctorType + "' AND status!='removed' AND status!='Completed' AND schedule_id!='" + prevScheduleId + "' ";
        ...
        {
            int appIDToRemove = AppointmentId;

            DialogResult result = MessageBox.Show("Are you sure you want to Cancel this Appointment..?", "Confirm Removal", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);

            if (result == DialogResult.OK)
            {
                Appointment app = new Appointment();
                app.APPOINTID = appIDToRemove;

                app.CancelAppointment(con); // Call the remove function

                MessageBox.Show("Appointment Booking cancelled successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Hide();
            }
            else
            {
                MessageBox.Show("Appointment Booking is not cancelled...", "Canceled", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
